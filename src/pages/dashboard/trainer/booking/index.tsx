...
const participants = Array.from(new Set(mockShifts.map((s) => s.participant))).filter(Boolean);
...
export default function TrainerPreviousShiftsPage() {
  ...
  const filteredShifts = previousShifts.filter((s) => {
    if (participantFilter && s.participant !== participantFilter) return false;
    if (statusFilter && s.status !== statusFilter) return false;
    if (
      monthFilter !== null &&
      !isSameMonth(parseISO(s.date), new Date(2025, monthFilter))
    )
      return false;
    if (
      search &&
      !(`${s.title} ${s.notes || ""}`.toLowerCase().includes(search.toLowerCase()))
    )
      return false;
    return true;
  });
